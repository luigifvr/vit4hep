...
class BaseExperiment:
    ...
    def _init_experiment(self):
        ...
        if not self.warm_start:
            if self.cfg.run_name is None:
                modelname = self.cfg.exp_type
                rnd_number = np.random.randint(low=0, high=99999)
                run_name = f"{modelname}_{rnd_number:05}"
            else:
    ...
    def train(self):
        ...
        for step in range(self.cfg.training.iterations):
            ...
            if step in [0, 9, 999] or (step + 1) % self.cfg.training.validate_every_n_steps == 0:
    ...
    def _step(self, data, step):
        ...
        grad_norm_net = (
            torch.nn.utils.clip_grad_norm_(self.model.net.parameters(), float("inf")).cpu().item()
        )
