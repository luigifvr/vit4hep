...
from experiments.calochallenge.freia_utils import get_coupling_block, get_permutation_block, get_block_kwargs

class CaloChallengeCINN(CINN):
    def __init__(self, coupling_block, nblocks, is_spatial, spatial_factor, cinn_kwargs, vit_kwargs, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.nblocks = nblocks
        self.CouplingBlock = get_coupling_block(coupling_block)
        self.PermuteBlock = get_permutation_block(is_spatial)
        self.block_kwargs = get_block_kwargs(is_spatial, self.shape, self.patch_shape, spatial_factor, cinn_kwargs, vit_kwargs)
