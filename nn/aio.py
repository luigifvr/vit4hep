...
class AllInOneBlock(InvertibleModule):
    ...
    def __init__(self, dims_in, dims_c=[],
        ...
        channels = dims_in[0][0][0]
        # rank of the tensors means 1d, 2d, 3d tensor etc.
        self.input_rank = len(dims_in[0][0]) - 1
        ...
        split_len2 = channels - channels // 2
        split_len1 = channels // 2
        ...
        self.global_scale = nn.Parameter(torch.ones(1, self.in_channels, *([1] * self.input_rank), dtype=torch.float) * float(global_scale))
        self.global_offset = nn.Parameter(torch.zeros(1, self.in_channels, *([1] * self.input_rank), dtype=torch.float))
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        if not rev:
            #a1 = self.subnet(x1c)
            a1 = self.subnet(x1, c)
            x2, j2 = self._affine(x2[:,0], a1)
        else:
            a1 = self.subnet(x1, c)
            #a1 = self.subnet(x1c)
            x2, j2 = self._affine(x2[:,0], a1, rev=True)

        log_jac_det = j2
        x_out = torch.cat((x1[:,0], x2), 1)
        ...
        x_out = x_out.reshape(-1, 1, 45, 16, 9)
