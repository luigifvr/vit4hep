...
class CaloCubicSplineBlock(fm.InvertibleModule):
    ...
    def __init__(
        ...
        self.bounds = self.bounds_activation(
            torch.ones(1, self.splits[1], *([12] * self.input_rank)) * float(bounds)
        ).reshape(-1, 270 * 12)
        self.tails = tails
        ...
        self.subnet = subnet_constructor(
            self.splits[0], (2 * self.num_bins + 2)
        )  # * self.splits[1])
        self.last_jac = None
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        if not rev:
            theta = self.subnet(x1, c).reshape(
                x1.shape[0], self.splits[1] * 12, 2 * self.num_bins + 2
            )
            x2, j2 = self._unconstrained_cubic_spline(
                x2.reshape(-1, 270 * 12), theta, rev=False
            )
            ...
            x2, j2 = self._unconstrained_cubic_spline(
                x2.reshape(-1, 270 * 12), theta, rev=True
            )
            x2 = x2.reshape(-1, x1.shape[1], x1.shape[2])
...
class CaloRationalQuadraticSplineBlock(fm.InvertibleModule):
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        if not rev:
            theta = self.subnet(x1, c).reshape(
                x1.shape[0], self.splits[1] * 12, 3 * self.num_bins - 1
            )
            x2, j2 = self._unconstrained_rational_quadratic_spline(
                x2.reshape(-1, 270 * 12), theta, rev=False
            )
