...
class BaseCalo2OneSideCouplingBlock(InvertibleModule):
    ...
    def __init__(self, dims_in, dims_c=[],
        ...
        if self.spatial:
            self.indices1 = [int(2*k) for k in range(self.channels //2) ]#[int(2*k) for k in range(self.channels // 2)]
            self.indices2 = [int(2*k+1) for k in range(self.channels // 2)]
        else:
            self.indices1 = [int(k) for k in range(self.channels //2) ]#[int(2*k) for k in range(self.channels // 2)]
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        if self.spatial:
            ...
            #y[:, ::2] = y1
            #y[:, 1::2] = y2
        #y = torch.cat([torch.stack([y11, y22], 0) for y11, y22 in zip(y1, y2)], 1) #TODO can be faster
        return (y,), j1
