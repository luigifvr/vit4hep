...
class ViT(nn.Module):
    ...
    def __init__(self, param):
        ...
        if self.learn_pos_embed:
            ...
            self.register_buffer('grid', torch.arange(self.prod_num_patches)/self.prod_num_patches)
        ...
        self.final_layer = FinalLayer(self.hidden_dim, self.patch_dim, self.out_channels, self.x_out)
...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, patch_dim, out_channels, x_out=None):
        ...
        self.linear = nn.Linear(hidden_dim, x_out*patch_dim) #math.prod(patch_shape) * out_channels )
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_torch_sdpa:
            ...
            k = k.transpose(1,2)
            v = v.transpose(1,2)
            x = memory_efficient_attention(q.contiguous(), k.contiguous(), v.contiguous(), p=self.attn_drop.p if self.training else 0.)
            x = x.transpose(1,2)
