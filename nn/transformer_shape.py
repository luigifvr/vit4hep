        ...
        if not rev:
            ...
            embedding = self.transformer(
                ...
                tgt=self.compute_embedding(
                    ...
                    embedding_net=self.x_embed_net,
                ),
                tgt_mask=torch.ones(
            ...
            if self.layer_cond:
                layer_one_hot = torch.eye(self.n_energy_layers, device=x.device).repeat(
                    len(t), 1
                )
                embedding = torch.cat([embedding, layer_one_hot], dim=1)
            pred = self.subnet(x_t, t.reshape((-1, 1)), embedding)
            pred = pred.unflatten(
                0, (-1, self.n_energy_layers)
            )  # (b l) c x y -> b l c x y

        else:
            x = torch.zeros(
                (len(c), 1, *self.shape[1:]), device=c.device, dtype=c.dtype
            )
            ...
            for i in range(self.n_energy_layers):
                ...
                if self.layer_cond:
                    layer_one_hot = repeat(
                        F.one_hot(
                            torch.tensor(i, device=x.device), self.n_energy_layers
                        ),
                        "d -> b 1 d",
                        b=len(c),
                    )
