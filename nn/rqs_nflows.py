...
class RationalQuadraticSplineBlock(BaseCalo2CouplingBlock):
    def __init__(self, dims_in, dims_c, subnet_constructor, num_bins=10, bounds_init=1.0,
                  tails="linear", bounds_type="SOFTPLUS", spatial=False, *args):
        super().__init__(dims_in, dims_c, spatial=spatial, *args)

        self._spline1 = RationalQuadraticSpline(dims_in, dims_c, subnet_constructor=subnet_constructor,
                                                  num_bins=num_bins, bounds_init=bounds_init, tails=tails,
                                                  bounds_type=bounds_type, spatial=spatial)
        
        self._spline2 = RationalQuadraticSpline(dims_in, dims_c, subnet_constructor=subnet_constructor,
                                                  num_bins=num_bins, bounds_init=bounds_init, tails=tails,
                                                  bounds_type=bounds_type, spatial=spatial)
...
class RationalQuadraticSpline(InvertibleModule):
    ...
    def __init__(self, dims_in, dims_c=[],
        ...
        self.subnet = subnet_constructor(self.splits[0], (3 * self.num_bins - 1))# * self.splits[1])
    ...
    def forward(self, x1, x2, c=[], rev=False):
        ...
        if not rev:
            theta = self.subnet(x1c, c).reshape(x1c.shape[0], -1, 3*self.num_bins - 1)#self.splits[1], -1, 3*self.num_bins - 1)
